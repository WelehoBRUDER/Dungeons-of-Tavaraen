function lisaaSpreadSheet(graafiBoxi, taulu, riviNumero, click) {
  ...
  if(!lisaaSpreadSheet.spreadSheetTree) lisaaSpreadSheet.spreadSheetTree = new Map();
  ...
  function lajittele(sarakeNum, jarjestys) {
    if(jarjestys == "ba") return luoTauluElementti(lisaaSpreadSheet.spreadSheetTree.get(graafiAvain).taulu);

    const taulu = JSON.parse(JSON.stringify(lisaaSpreadSheet.spreadSheetTree.get(graafiAvain).taulu));
    const otsikko = taulu[0].slice();
    let pohja = taulu.slice(1);

    if(!jarjestys) {otsikko[sarakeNum].jarjestys = "ab"} 
    else if(jarjestys == "ab") {otsikko[sarakeNum].jarjestys = "ba"}
  ...
  function resize() {
    ...
    const leveys = tauluBox.getBoundingClientRect().width < table.getBoundingClientRect().width;
