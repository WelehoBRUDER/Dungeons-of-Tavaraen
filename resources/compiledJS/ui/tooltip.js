...
function abiTT(abi, character = player, options) {
    ...
    if (abi.self_target)
        txt += `<f>${fontSize}px<f>${lang["targets_self"]}\n`;
    if (abi.mana_cost > 0)
...
function compareAbilityTooltip(ability, character, bonuses) {
    ...
    if (abi.use_range > 0) {
        ...
        if (compare.use_rangeV) {
            txt += `<i>${icons.range}<i><f>16px<f><c>white<c>${lang["use_range"]}: <c>lime<c>[${value} ---> ${value + compare.use_rangeV}] <c>white<c>${lang["tiles"]}\n`;
        }
        else {
    ...
    if (abi.shoots_projectile != "")
        txt += `<f>16px<f>${lang["ranged"]}\n`;
    if (abi.requires_melee_weapon)
        txt += `<i>${icons.melee}<i><f>16px<f>${lang["requires_melee_weapon"]}\n`;
    else if (abi.requires_ranged_weapon)
        txt += `<i>${icons.ranged}<i><f>16px<f>${lang["requires_ranged_weapon"]}\n`;
    if (abi.requires_concentration)
        txt += `<i>${icons.concentration}<i><f>16px<f>${lang["concentration_req"]}\n`;
    if (abi.recharge_only_in_combat)
        txt += `<i>${icons.fighter_symbol}<i><f>16px<f>${lang["recharge_only_in_combat"]}\n`;
    if (abi.summon_unit)
    ...
    if (abi.self_target)
        txt += `<f>16px<f>${lang["targets_self"]}\n`;
    if (abi.mana_cost > 0) {
