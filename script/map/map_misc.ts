function spriteVariables() {
  ...
  const mapOffsetStartX =
    player.cords.x - settings.map_offset_x - Math.floor(spriteLimitX / 2);
  const mapOffsetStartY =
    player.cords.y - settings.map_offset_y - Math.floor(spriteLimitY / 2);
