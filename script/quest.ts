  ...
  if(entry.obj >= quest.objectives.length) { text += "\n<f>54px<f><c>goldenrod<c>This quest is complete!"; // Make it display all objectives later, for now this is a placeholder. 
    ...
    if(currentTask.objective == "killEnemies") {
      if(currentTask.spawnUnique) {
        uniqueQuestSpawns[currentTask.spawnUnique].forEach((enemy: any, index: any) => {
          let col = "silver";
          if(entry.prog[index]) col = "lime";
          text += `\n<c>${col}<c>(${entry.prog[index] ?? 0}/1) Defeat ${lang[enemy.enemy + "_name"]}`;
        });
      }
    }
...
function updateQuestProgress(data: any, npc: string = "") {
  ...
  if(objective.objective == "killEnemies") {
    if(objective.spawnUnique) {
      ...
      if(questId.prog[0] >= objective.amountNeeded) isObjectiveComplete = true;
    }
  }
