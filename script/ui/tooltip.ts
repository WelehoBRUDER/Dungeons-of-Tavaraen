...
function abiTT(abi: ability, character: any = player, options?: { fontSize?: number }) {
	...
	if (abi.use_range > 0) txt += `<i>${icons.range}<i><f>${fontSize}px<f>${lang["use_range"]}: ${abi.use_range} ${lang["tiles"]}\n`;
	...
	if (abi.shoots_projectile != "") txt += `<f>${fontSize}px<f>${lang["ranged"]}\n`;
	if (abi.requires_melee_weapon) txt += `<i>${icons.melee}<i><f>${fontSize}px<f>${lang["requires_melee_weapon"]}\n`;
	else if (abi.requires_ranged_weapon) txt += `<i>${icons.ranged}<i><f>${fontSize}px<f>${lang["requires_ranged_weapon"]}\n`;
	if (abi.requires_concentration) txt += `<i>${icons.concentration}<i><f>${fontSize}px<f>${lang["concentration_req"]}\n`;
	if (abi.recharge_only_in_combat) txt += `<i>${icons.fighter_symbol}<i><f>${fontSize}px<f>${lang["recharge_only_in_combat"]}\n`;
	if (abi.summon_unit)
	...
	if (abi.self_target) txt += `<f>${fontSize}px<f>${lang["targets_self"]}\n`;
...
function compareAbilityTooltip(ability: ability, character: any, bonuses: any) {
	...
	if (abi.damage_multiplier) {
		...
		if (compare.damage_multiplierP) {
			...
			txt += `<i>${icons.damage}<i><f>16px<f><c>white<c>${lang["damage_multiplier"]}: <c>${change > 0 ? "lime" : "red"}<c>[${value}% ---> ${
				value + change
			}%]<c>white<c>\n`;
		} else {
	...
	if (abi.resistance_penetration || compare.resistance_penetrationV) {
		...
		if (compare.resistance_penetrationV) {
			txt += `<i>${icons.rp}<i><f>16px<f><c>white<c>${lang["resistance_penetration"]}: <c>${
				change > 0 ? "lime" : "red"
			}<c>[${value}% ---> ${value + change}%]<c>white<c>\n`;
		} else {
			txt += `<i>${icons.rp}<i><f>16px<f>${lang["resistance_penetration"]}: ${value}%\n`;
		}
	}
	if (abi.use_range > 0) {
		const value = abi.use_range;
	...
	if (abi.shoots_projectile != "") txt += `<f>16px<f>${lang["ranged"]}\n`;
	if (abi.requires_melee_weapon) txt += `<i>${icons.melee}<i><f>16px<f>${lang["requires_melee_weapon"]}\n`;
	else if (abi.requires_ranged_weapon) txt += `<i>${icons.ranged}<i><f>16px<f>${lang["requires_ranged_weapon"]}\n`;
	if (abi.requires_concentration) txt += `<i>${icons.concentration}<i><f>16px<f>${lang["concentration_req"]}\n`;
	if (abi.recharge_only_in_combat) txt += `<i>${icons.fighter_symbol}<i><f>16px<f>${lang["recharge_only_in_combat"]}\n`;
	if (abi.summon_unit)
	...
	if (abi.self_target) txt += `<f>16px<f>${lang["targets_self"]}\n`;
	if (abi.mana_cost > 0) {
		if (compare.mana_costV) {
			const value = abi.mana_cost;
			const change = compare.mana_costV;
			txt += `<i>${icons.mana}<i><f>16px<f><c>white<c>${lang["mana_cost"]}: <c>${change < 0 ? "lime" : "red"}<c>[${value} ---> ${
				value + change
			}]<c>white<c>\n`;
		} else {
	...
	if (abi.health_cost > 0 || abi.health_cost_percentage > 0) {
		if (abi.health_cost > 0) {
			if (compare.health_costV) {
				...
				txt += `<f>16px<f><i>${icons.health_cost}<i><c>white<c>${lang["health_cost"]}: <c>${change < 0 ? "lime" : "red"}<c>[${value} ---> ${
					value + change
				}]<c>white<c>\n`;
			} else {
			...
			if (compare.health_cost_percentageV) {
				...
				txt += `<f>16px<f><i>${icons.health_cost}<i><c>white<c>${lang["health_cost"]}: <c>${
					change < 0 ? "lime" : "red"
				}<c>[${value}% ---> ${value + change}%]<c>white<c>\n`;
			} else {
	...
	if (abi.cooldown > 0) {
		if (compare.cooldownV) {
			...
			txt += `<i>${icons.cooldown}<i><f>16px<f><c>white<c>${lang["cooldown"]}: <c>${change < 0 ? "lime" : "red"}<c>[${value} ---> ${
				value + change
...
function statTT(status: statEffect, options?: { embed?: boolean; container?: boolean }) {
	...
	if (!options?.embed) txt += `\t<f>26px<f>${helper.localise("effect_" + status.id + "_name")}\t\n`;
	if (!options?.embed) txt += `<f>18px<f><c>silver<c>"${lang["effect_" + status.id + "_desc"] ?? status.id + "_desc"}"\t\n`;
	if (Object.keys(status.dot).length > 0) {
		txt += `§${options?.embed ? " " : ""}<f>${options?.embed ? "16px" : "${fontSize}px"}<f>${helper.localise("deals")} ${
			status.dot.damageAmount
		} <i>${status.dot.icon}<i>${helper.localise(status.dot.damageType).toLowerCase()} ${helper.localise("damage").toLowerCase()}\n`;
	}
	...
	if (Object.keys(status.dot).length > 0) {
		...
		if (bonuses.dot) {
			txt += `§<f>"16px"<f><c>white<c>${lang["deals"]} <c>lime<c>[${value} ---> ${value + bonuses.dot.damageAmount}] <c>white<c><i>${
				status.dot.icon
			}<i>${helper.localise(status.dot.damageType).toLowerCase()} ${lang["damage"].toLowerCase()}\n`;
		} else {
			txt += `§<f>"16px"<f>${lang["deals"]} ${value} <i>${status.dot.icon}<i>${helper.localise(status.dot.damageType).toLowerCase()} ${lang[
				"damage"
			].toLowerCase()}\n`;
		}
	}
